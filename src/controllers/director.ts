...
// Get All Directors
export const getAllDirectors = async (req: AuthRequest, res: Response) => {
  try {
    const directorList = await Director.aggregate([
        {
          $project: {
            name: 1,
            _id: 1,
    ...
    res.status(200).json({ 
        success: true, 
        message: "Director list with name and id",
        data: { directorList }
    });
  } catch (error) {
    res.status(500).json({
...
export const addOrUpdateDirector = async (req: AuthRequest, res: Response) : Promise< string | any > => {
  try {
    ...
    const file = req?.files?.image as UploadedFile;
     ...
     const directorPayload = getValidDirectorPayload(req.body, existingDirector ? true : false);

    // corner-cases error handling 
    if(!existingDirector && directorId) {
        throw new Error("Director not found in DB");
      }
      if(existingDirector && Object.keys(directorPayload).length === 0 && !file) {
        throw new Error("Atleast one field required to update Director information");
      }
...
export const deleteDirector = async (req: AuthRequest, res: Response) : Promise< string | any > => {
  try {
     // Check if the user is an admin
    if (!req.user || req.user.role !== "admin") {
      return res.status(403).json({ message: "Access denied. Admins only." });
