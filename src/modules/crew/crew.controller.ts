...
import fs from "fs";
import { getValidCrewPayload } from "../../utils/getPayload";
import { ADMIN } from "../../utils/constants";

//Get searched Crew by name--------------------------------------------------------------------------------
export const searchCrewByName = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
...
export const addOrUpdateCrew = async (
  req: AuthRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    const castPayload = getValidCrewPayload(
      req.body,
      existingCrew ? true : false
    );
